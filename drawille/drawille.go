...
// SetText Set text to the given coordinates
...
func (c Canvas) Rows(minX, minY, maxX, maxY int) []string {
	...
	for rownum := minrow; rownum < (maxrow + 1); rownum = rownum + 1 {
		...
		for x := mincol; x < (maxcol + 1); x = x + 1 {
			...
			row += string(rune(char + brailleCharOffset))
		}
		ret = append(ret, row)
	}
	return ret
...
// DrawPolygon aaa
func (c *Canvas) DrawPolygon(centerX, centerY, sides, radius float64) {
	degree := 360 / sides
	for n := 0; n < int(sides); n = n + 1 {
		a := float64(n) * degree
		b := float64(n+1) * degree

		x1 := int(centerX + (math.Cos(radians(a)) * (radius/2 + 1)))
		y1 := int(centerY + (math.Sin(radians(a)) * (radius/2 + 1)))
		x2 := int(centerX + (math.Cos(radians(b)) * (radius/2 + 1)))
		y2 := int(centerY + (math.Sin(radians(b)) * (radius/2 + 1)))
